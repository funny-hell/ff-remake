...
const vec4  light_color = vec4(1.0, 0.11, 0.0, 0.3); // r, g, b, a. the gradient light that is above (and on top) of the water
const vec4  glow_color = vec4(1.0, 0.80, 0.0, 0.5);  // alpha is unused here. the color that white parts glow when near water
const float glow_strength = 0.9;                      // how strong the glow effect is
const float glow_threshold = 0.00;                   // how bright the green channel needs to be to trigger a glow
const float glow_distance = .0005;                   // size of the glow
...
void frag()
{
    ...
    float dy = waterheight - worldPos.y + 400;
