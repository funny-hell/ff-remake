...
varying vec2 worldPos;
    ...
    void frag(){
        ...
        vec2 rippleval = vec2(sin(-timer*1.9+worldPos.y / 9.0) * .0007 * ratio.x, 0.0);
        rippleval += vec2(sin(-timer*1.7+worldPos.y / 16.0) * .0005 * ratio.x, 0.0);
        rippleval += vec2(sin(-timer*1.4+worldPos.x / 32.0) * .0006 * ratio.x, 0.0);
		...
		reflectionval += vec2(0.0, cos(-timer*3+worldPos.y / 2.0) * .005 * ratio.y - 0.01); //ripple distortion
		
        vec4 reflection = texture(framebuf, screencoords.xy - reflectionval);

        float ripple_alpha = .55;
        float ripple_fadedown = 4.0;

		// vec2(-0.04+0.01 / screencoords.y, 0.0) //tilt
